...
def psar(d,step=.02,max_step=.2):
 """Wilder/TradingView PSAR. Reversal is tested on the free candidate BEFORE the two-bar guard."""
 ...
 for i in range(2,n):
  ...
  if u:
   # TradingView/Wilder: test current-bar penetration first.
   if l[i]<q:
    u=False;q=ep;ep=l[i];af=step
   else:
    q=min(q,l[i-1],l[i-2])
    if h[i]>ep:ep=h[i];af=min(max_step,af+step)
  else:
   if h[i]>q:
    u=True;q=ep;ep=h[i];af=step
   else:
    q=max(q,h[i-1],h[i-2])
    if l[i]<ep:ep=l[i];af=min(max_step,af+step)
  s[i]=q;b[i]=u
...
def event_rows(sym,z):
 ...
 for i in np.flatnonzero(flip.values):
  ...
  for hh in [5,10,20,30,60]:
   r[f'immediate_r{hh}']=side*(z.close.iloc[i+hh]/entry-1) if i+hh<len(z) else np.nan
   r[f'retest_r{hh}']=side*(z.close.iloc[hit+hh]/z.close.iloc[hit]-1) if hit is not None and hit+hh<len(z) else np.nan
  rows.append(r)
 return rows
def summarize(ev):
 ...
 for side in ['LONG','SHORT']:
  ...
  for name,f in filters.items():
   g=ev[base&f]
   for hh in [5,10,20,30,60]:
    x=g[f'immediate_r{hh}'].dropna();rt=g[g.retest5][f'retest_r{hh}'].dropna();rows.append({'side':side,'filter':name,'horizon':hh,'n':len(x),'win':(x>0).mean() if len(x) else np.nan,'mean':x.mean(),'median':x.median(),'retest_rate':g.retest5.mean() if len(g) else np.nan,'retest_n':len(rt),'retest_win':(rt>0).mean() if len(rt) else np.nan,'retest_mean':rt.mean()})
 return pd.DataFrame(rows)
def run_all():
 OUT.mkdir(exist_ok=True);allr=[];fails={}
 for s0 in ASSETS:
  try:d=fetch(s0);z=indicators(d).join(weekly(d));allr+=event_rows(s0,z)
  except Exception as e:fails[s0]=str(e);print('FAIL',s0,e)
