...
for s in ASSETS:
 ...
 except Exception as e:fail[s]=str(e);print('FAIL',s,e)
# BTC regime and relative-strength features use only contemporaneous/past observations.
...
out=pd.DataFrame(rows).sort_values(['test_win10','test_n'],ascending=False);print('ASSETS OK',len(frames),'FAIL',fail,'SPLIT',cut);print(out.head(50).to_string(index=False));out.to_csv(OUT/'search80_expanded.csv',index=False)
# Frozen benchmark from prior 7-asset experiment, evaluated across expanded universe and calendar subperiods.
bench=('weekly_bull','adx20','adx_up','rvol1_5','break20');print('\nFROZEN BENCHMARK',bench)
for a,b in [('2018','2020'),('2021','2022'),('2023','2024'),('2025','2026')]:
 q=df.loc[a:b];n,w,m,md=ev(q,bench);print(a,b,'n',n,'win10',round(float(w),4) if n else None,'mean10',round(float(m),4) if n else None,'median10',round(float(md),4) if n else None)
