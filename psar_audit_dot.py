...
def get():
 ...
 for c in ['o','h','l','c']:d[c]=pd.to_numeric(d[c]);d['date']=pd.to_datetime(d.ot,unit='ms',utc=True);return d
...
def tvsar(d,start=.02,inc=.02,maxaf=.2):
 ...
 # TradingView reference initialization: on bar 1 infer direction from closes; SAR=prior extreme; EP=current extreme.
