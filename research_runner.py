...
Goal: identify robust TAKE/SKIP filters for bullish daily PSAR flips.
Exit rule remains the next bearish daily PSAR flip.
Runs a battery of feature/regime/asset-family experiments in one process and writes leaderboard CSV.
"""
...
def feats(s):
 ...
 z['atrp']=z.atr/c; z['sar_gap']=(c-z.psar)/z.atr; z['di_gap']=z.plus_di-z.minus_di
 z['bb_mid']=c.rolling(20).mean();sd=c.rolling(20).std();z['bb_width']=4*sd/z.bb_mid
 z['obv']=(np.sign(c.diff()).fillna(0)*v).cumsum();mf=((c-z.low)-(z.high-c))/(z.high-z.low).replace(0,np.nan)*v;z['cmf20']=mf.rolling(20).sum()/v.rolling(20).sum()
 z['vol_z']=(v-v.rolling(20).mean())/v.rolling(20).std();z['dd50']=c/c.rolling(50).max()-1
 z['ema_stack']=(c>z.ema20)&(z.ema20>z.ema50)&(z.ema50>z.ema200)
 bull=z.psar_bull.astype(bool); en=bull&~bull.shift(1,fill_value=False); ex=~bull&bull.shift(1,fill_value=False);E=np.flatnonzero(en);X=np.flatnonzero(ex)
 bear=~bull;age=bear.groupby(bear.ne(bear.shift()).cumsum()).cumcount()+1;rows=[]
 for i in E:
  q=[j for j in X if j>i]
  if not q or i<90:continue
  j=q[0];r={'date':z.index[i],'symbol':s,'ret':float(c.iloc[j]/c.iloc[i]-1),'days':j-i}
  def val(col,k=1):return float(z[col].iloc[i-k]) if pd.notna(z[col].iloc[i-k]) else np.nan
  r.update({'bear_age':float(age.iloc[i-1]),'adx':val('adx'),'adx_d3':val('adx')-val('adx',4),'di_gap':val('di_gap'),'di_d3':val('di_gap')-val('di_gap',4),'minus_d3':val('minus_di')-val('minus_di',4),'rsi':val('rsi'),'rsi_d3':val('rsi')-val('rsi',4),'hist':val('macd_hist'),'hist_d3':val('macd_hist')-val('macd_hist',4),'hist_d7':val('macd_hist')-val('macd_hist',8),'rvol':val('rvol20'),'vol_z':val('vol_z'),'atrp':val('atrp'),'atr_d5':val('atrp')-val('atrp',6),'bbw':val('bb_width'),'bbw_d5':val('bb_width')-val('bb_width',6),'cmf':val('cmf20'),'obv_d10':val('obv')-val('obv',11),'sar_gap':val('sar_gap'),'dd50':val('dd50'),'weekly':bool(z.weekly_bull.iloc[i]) if pd.notna(z.weekly_bull.iloc[i]) else False,'stack':bool(z.ema_stack.iloc[i-1])})
...
leader=[];checked=0
# Search compact rules; cap complexity to reduce multiple-testing/overfit risk.
for k in range(1,6):
 for ids in combinations(range(len(names)),k):
  checked+=1;m=A[:,ids].all(1);n,w,mu,md=st(m,'tr');nv,wv,muv,mdv=st(m,'va')
  if n<60 or nv<20 or min(w,wv)<.50:continue
  nt,wt,mut,mdt=st(m,'te')
  if nt<20:continue
  leader.append({'rule':'+'.join(names[i] for i in ids),'k':k,'train_n':n,'train_win':w,'valid_n':nv,'valid_win':wv,'test_n':nt,'test_win':wt,'test_mean':mut,'test_median':mdt,'robust_min':min(w,wv,wt)})
 print('PROGRESS|k=%d|checked=%d|kept=%d'%(k,checked,len(leader)),flush=True)
L=pd.DataFrame(leader).sort_values(['robust_min','test_win','test_mean'],ascending=False);L.to_csv(OUT/'sar_v2_leaderboard.csv',index=False)
print('BASE|n=%d|win=%.4f|mean=%.4f|days=%.2f|split=%s,%s'%(len(t),(y>0).mean(),y.mean(),t.days.mean(),c1.date(),c2.date()),flush=True)
